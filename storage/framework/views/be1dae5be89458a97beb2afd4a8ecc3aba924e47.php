...
<div class="profile">
	<div class="container">
		<div class="breadcrumbs"><a href="/">HOME</a> / <?php echo e($profileUser->name); ?></div>
	</div>
			...
			<div class="col-md-12">
				<div class="profile__info">
					...
					<h3>
						...
						<div class="status" style=" background-color: <?php echo e(Auth::check() ?  '#1CFF0C' : 'red'); ?> "></div>
					</h3>

					<?php if(Auth::user()->name == $profileUser->name ): ?>
						<div> 
						 <?php if($profileUser->blocked ==  0): ?>
							<a href="/threads/create">Created Thread</a> 
						  <?php elseif($profileUser->blocked ==  1): ?>
						    <p>You have been blocked</p>
						  <?php endif; ?>
						</div>
						<a href="<?php echo e(url('/profiles/'.Auth::user()->slug.'/edit')); ?>"><i class="fa fa-cog" aria-hidden="true"></i></a> <?php endif; ?>
						<?php if(Auth::user()->name != $profileUser->name ): ?>
							<div>
								<following :isfollowing="<?php echo e(json_encode(Follower::yourFollowing($profileUser->id))); ?>" :profileuser="<?php echo e(json_encode($profileUser->id)); ?>"></following>
							</div>
						<?php endif; ?>
				</div>
				...
				<div class="profile__comments">
					...
					<div class="profile__activities">
						<?php if($activities != ''): ?> <?php $__empty_1 = true; $__currentLoopData = $activities; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $date => $activity): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?> <?php $__currentLoopData = $activity; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $record): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?> <?php if(view()->exists("profiles.activities.{$record->type}")): ?>
	<?php echo $__env->make("profiles.activities.{$record->type}", ["activity" => $record], array_except(get_defined_vars(), array('__data', '__path')))->render(); ?> <?php endif; ?> <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?> <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
