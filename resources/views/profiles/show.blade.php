			...
			<div class="row">
				<div class="col-md-12">
					<div class="profile__info">
						...
						<h3> 
							{{ $profileUser->name }}<div class="status" style=" background-color: {{ Auth::check() ?  '#1CFF0C' : 'red' }} "></div>
						</h3>

						@if(Auth::user()->name == $profileUser->name )
							<div> <a href="/threads/create">Created Thread</a> </div>
							<a href="{{ url('/profiles/'.Auth::user()->id.'/edit')}}"><i class="fa fa-cog" aria-hidden="true"></i></a>
						@endif

						@if(Auth::user()->name != $profileUser->name )
							<div>
								<following :isfollowing="{{ json_encode(Follower::yourFollowing($profileUser->id))  }}" :user="{{ json_encode($profileUser->id)}}"></following>
							</div>
						@endif	
					</div>	
